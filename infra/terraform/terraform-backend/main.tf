...
# Enable versinoning for the S3 bucket
#This ensures that we recover previous versions of the state file in case of accidental deletion or corrupt
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  ...
  tags = {
    Name        = "${var.project_name}-terraform_state_lock"
    Environment = var.environment
  }
}